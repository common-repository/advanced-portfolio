 ...
 *
 */
if ( ! function_exists( 'sp_advp_add_element' ) ) {
  function sp_advp_add_element( $field = array(), $value = '', $unique = '' ) {
    ...
    $class      = 'SP_AdvP_Framework_Option_' . $field['type'];
    ...
    if( class_exists( $class ) ) {
      ...
      $output .= '<p>'. __( 'This field class is not available!', 'advanced-portfolio' ) .'</p>';
    }
 ...
 *
 */
if ( ! function_exists( 'sp_advp_load_option_fields' ) ) {
  function sp_advp_load_option_fields() {

    $located_fields = array();

    foreach (glob(SP_AdvP_F_DIR .'/fields/*/*.php') as $sp_field) {
      $located_fields[] = basename($sp_field);
      sp_advp_locate_template(str_replace(SP_AdvP_F_DIR, '', $sp_field));
    }

    $override_name = apply_filters('sp_advp_framework_override', 'sp-framework-override');
    ...
    if( is_dir( $override_dir ) ) {
      ...
      foreach ( glob( $override_dir .'/*/*.php' ) as $override_field ) {
        ...
        if(! in_array(basename($override_field), $located_fields)) {
          ...
          sp_advp_locate_template(str_replace($override_dir, '/fields', $override_field));
    ...
    do_action('sp_advp_load_option_fields');
