 ...
 *
 */
class SP_AdvP_Framework extends SP_AdvP_Framework_Abstract {
    ...
    public static function instance( $settings = array(), $options = array() ) {
        if ( is_null( self::$instance ) && SP_AdvP_F_ACTIVE_FRAMEWORK ) {
    ...
    public function validate_save( $request ) {
        ...
        if ( isset( $request['import'] ) && ! empty( $request['import'] ) ) {
            ...
            $add_errors[] = $this->add_settings_error( esc_html__( 'Success. Imported backup options.', 'advanced-portfolio' ), 'updated' );
        }

        // reset all options
        if ( isset( $request['resetall'] ) ) {
            $add_errors[] = $this->add_settings_error( esc_html__( 'Default options restored.', 'advanced-portfolio' ), 'updated' );
            return;
        }
        ...
        if ( isset( $request['reset'] ) && ! empty( $section_id ) ) {
            ...
            $add_errors[] = $this->add_settings_error( esc_html__( 'Default options restored for only this section.', 'advanced-portfolio' ), 'updated' );
        }
    ...
    public function field_callback( $field ) {
        ...
        echo sp_advp_add_element($field, $value, $this->unique);
    }
    ...
    public function admin_page() {
        ...
        echo '<div class="sp-advp-framework sp-option-framework">';
        ...
        echo ( $this->settings['ajax_save'] ) ? '<span id="sp-save-ajax">'. esc_html__('Settings saved.', 'advanced-portfolio') .'</span>' : '';

        submit_button(esc_html__('Save Changes', 'advanced-portfolio'), 'primary sp-save', 'save', false, array( 'data-save' => esc_html__('Saving...', 'advanced-portfolio')));
        submit_button(esc_html__('Restore', 'advanced-portfolio'), 'secondary sp-restore sp-reset-confirm', $this->unique .'[reset]', false);

        if ($this->settings['show_reset_all']) {
            submit_button(esc_html__('Reset All Options', 'advanced-portfolio'), 'secondary sp-restore sp-warning-primary sp-reset-confirm', $this->unique .'[resetall]', false);
        }

        echo '</fieldset>';
        //echo ( empty( $has_nav ) ) ? '<a href="#" class="sp-expand-all"><i class="fa fa-eye-slash"></i> '. esc_html__( 'show all options', 'advanced-portfolio' ) .'</a>' : '';
        ...
        echo '</div>'; // end .sp-advp-framework
